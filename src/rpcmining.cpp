...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or voyacoin.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateVoyacoins(fGenerate, pwalletMain, nGenProcLimit);
    }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    uint256 hashTarget = uint256().SetCompact(pblock->nBits);
        ...
        // print to log
        LogPrintf("voyacoinMiner: proof-of-work found; hash: %s ; target: %s\n", hash.GetHex().c_str(), hashTarget.GetHex().c_str());
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("voyacoinMiner : generated block is stale");
        ...
        if (!ProcessBlockFound(pblock, wallet, reservekey))//!ProcessBlockFound(state, NULL, pblock))
            return error("voyacoinMiner : ProcessBlock, block not accepted");
    }
...
// NOTE: Unlike wallet RPC (which use VOYA values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.voyacoin.it/wiki/BIP_0022 for full specification.\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Voyacoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Voyacoin is downloading blocks...");
