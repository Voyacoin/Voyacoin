...
*Setup instructions for a gitian build of Voyacoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Voyacoin
...
- [Building Voyacoin](#building-voyacoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-voyacoin-host/README.md).
...
Clone the git repositories for voyacoin and gitian and then checkout the voyacoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/voyacoin/voyacoin
cd voyacoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the voyacoin repository
...
To build Voyacoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the voyacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/voyacoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/voyacoin/voyacoin
...
For example:
```bash
URL=https://github.com/laanwj/voyacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit voyacoin=${COMMIT} --url voyacoin=${URL} ../voyacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit voyacoin=${COMMIT} --url voyacoin=${URL} ../voyacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit voyacoin=${COMMIT} --url voyacoin=${URL} ../voyacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/voyacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/voyacoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/voyacoin-build.assert
```
...
[voyacoin/gitian.sigs](https://github.com/voyacoin/gitian.sigs/) repository, or if that's not possible create a pull
