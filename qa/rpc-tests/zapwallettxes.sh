...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars VOYACOIND and VOYACOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

VOYACOIND=${VOYACOIND:-${1}/voyacoind}
CLI=${VOYACOINCLI:-${1}/voyacoin-cli}
...
$VOYACOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$VOYACOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
