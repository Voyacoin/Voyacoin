...
export VOYACOINCLI=${BUILDDIR}/qa/pull-tester/run-voyacoin-cli
export VOYACOIND=${REAL_VOYACOIND}
...
if [ "x${ENABLE_VOYACOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and voyacoind must all be enabled"
fi
