...
###   This script attempts to download the signature file SHA256SUMS.asc from voyacoin.org
...
WORKINGDIR="/tmp/voyacoin"
TMPFILE="hashes.tmp"

#this URL is used if a version number is not specified as an argument to the script
SIGNATUREFILE="https://voyacoin.org/bin/0.9.2.1/SHA256SUMS.asc"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://voyacoin.org/bin/"
VERSIONPREFIX="voyacoin-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'voyacoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[voyacoin-]<version>-[rc[0-9]] (example: voyacoin-0.9.2-rc1)"
   echo "wget output:"
